...
contract PuzzleMasterTest is Test {
    ...
    function _createMerkleTree(address userA, address userB)
        internal
        pure
        returns (
            bytes32 root,
    ...
    function testClaimPrizeNoMerkle(uint256 amount) public {
        ...
        assertEq(
            ...
            amount,
            "Alice should have received WETH from prize claim."
        );

        // Bob tries to claim the same prize.
        vm.prank(bob);
        vm.expectRevert(
            abi.encodeWithSelector(PuzzleMaster.PuzzleMaster__NoPrize.selector)
        );
        master.claimPrize(proof, leaf, guess);

        // Make sure no prizes remain.
        (bytes32[] memory allPrizes, uint256[] memory amounts) = master
            .getActivePrizes();
    ...
    function testClaimPrizeWithMerkle(uint256 amount) public {
        ...
        (bytes32 root, bytes32 aliceLeaf, bytes32 bobLeaf) = _createMerkleTree(
            alice,
            bob
        );
    ...
    function testClaimPrizeWithMerkleFrontRun(uint256 amount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PuzzleMaster.PuzzleMaster__InvalidProof.selector
            )
        );
        master.claimPrize(proof, aliceLeaf, guess);
    }
    ...
    function testAddPrize(uint256 amount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PuzzleMaster.PuzzleMaster__PrizeAlreadySet.selector
            )
        );
        master.addPrize(prizeHash, amount);

        assertEq(
            WETH.balanceOf(address(master)),
            amount,
            "Puzzle master should have `amount` of WETH."
        );
        assertEq(
            master.prizes(prizeHash),
            amount,
            "Prize should have been recorded in Puzzle Master."
        );
        (bytes32[] memory allPrizes, uint256[] memory amounts) = master
            .getActivePrizes();
        ...
        assertEq(
            amounts[0],
            amount,
            "Prize amount should be in puzzle master."
        );
    ...
    function testRemovePrize(uint256 amount) public {
        ...
        assertEq(
            ...
            amount,
            "Caller should have `amount` of WETH."
        );
        assertEq(
            WETH.balanceOf(address(master)),
            0,
            "Puzzle master should have zero WETH."
        );

        assertEq(
            master.prizes(prizeHash),
            0,
            "Prize should have been removed from Puzzle Master."
        );
        (bytes32[] memory allPrizes, uint256[] memory amounts) = master
            .getActivePrizes();
        assertEq(allPrizes.length, 0, "There should be no prizes left.");
    ...
    function testShutdown(uint256 amount) public {
        ...
        assertEq(
            WETH.balanceOf(address(this)),
            4 * amount,
            "Caller should have got all the WETH from puzzle master."
        );
        assertEq(
            WETH.balanceOf(address(master)),
            0,
    ...
    function testMultiplePrizesMultipleUsers(uint8 salt) public {
        ...
        (bytes32 root, bytes32 leafA, bytes32 leafB) = _createMerkleTree(
            alice,
            bob
        );
        master.changeRoot(root);
        ...
        for (uint256 i = 0; i < 4; i++) {
            assertEq(
                allPrizes[i],
                master.getHash(guesses[i]),
                "Prize hash differs from expected."
            );
            assertEq(
                amounts[i],
                prizes[i],
                "Prize amount differs from expected."
            );
        }
        bytes32[] memory proof = new bytes32[](1);
        ...
        assertEq(
            WETH.balanceOf(alice),
            prizes[0],
            "Alice should have received the first prize."
        );
        ...
        assertEq(
            WETH.balanceOf(bob),
            prizes[1],
            "Bob should have received the second prize."
        );
        ...
        assertEq(
            WETH.balanceOf(sally),
            prizes[2],
            "Sally should have received the third prize."
        );
        ...
        assertEq(
            WETH.balanceOf(alice),
            prizes[0] + prizes[3],
            "Alice should have received the last prize."
        );
        ...
        assertEq(
            WETH.balanceOf(address(master)),
            0,
            "Puzzle master should have no more WETH in it."
        );
    }
}
