...
/**
 * @title Puzzle Master
 * @notice Stores keccak256 hashes, and allows users to claim prizes if they know the pre-image to those hashes.
...
contract PuzzleMaster is Ownable {
    ...
    function claimPrize(
        ...
        if (root != bytes32(0)) {
            ...
            ) revert PuzzleMaster__InvalidProof();
        }
    ...
    function addPrize(bytes32 prizeHash, uint256 prizeAmount)
        external
        onlyOwner
    {
    ...
    function getPrizeAmount(string memory guess)
        external
        view
    ...
    function getActivePrizes()
        external
        view
        returns (bytes32[] memory allPrizes, uint256[] memory amounts)
    {
